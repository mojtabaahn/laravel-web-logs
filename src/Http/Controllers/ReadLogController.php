...
{
    public function __invoke(string $filename, ReadLastLines $readLines)
    {
        ...
        $contents = $readLines($path, config('web-logs.max_read_lines'));

        $matches = preg_split("/\[([0-9]{4}-[0-9]{2}-[0-9]{2}\s[0-9]{2}:[0-9]{2}:[0-9]{2})\]/s", $contents, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);

        if (preg_match("/([0-9]{4}-[0-9]{2}-[0-9]{2}\s[0-9]{2}:[0-9]{2}:[0-9]{2})/s", $matches[0]) === 0) {
            array_shift($matches);
        }
        $temp = null;
